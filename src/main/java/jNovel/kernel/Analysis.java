    ...
    public static UrlData analysisUrl(String data) {
        ...
        if (A.indexOf("forum.php?") >= 0) {
            ...
            for (int n = 0; n < temp2.length; n++) {
                if (temp2[n].equals("tid")) result.Tid = Integer.parseInt(temp2[n + 1]);
                if (temp2[n].equals("page")) result.page = Integer.parseInt(temp2[n + 1]);

            }
            result.wrongUrl = false;
        }
        else if (A.matches("thread-[0-9]+-[0-9]+-[0-9A-Z]+.html")) {
            String[] temp2 = A.split("-");
